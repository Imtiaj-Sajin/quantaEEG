...
**Double-anonymous review: chosen.** JNE lets authors choose; the
corresponding author chose double-anonymous on 2026-09-15. Never hand-strip
`main.tex`: build `main_anonymous.tex`, or upload the generated
`submission_anonymous.zip`. Details under "Upload archives" below.
...
Submission is **double-anonymous** (chosen 2026-09-15). Three uploads, all in
`paper/build/` and all regenerated by two commands:

```bash
python paper/make_overleaf_zip.py --check   # submission_anonymous.zip + manuscript_named.zip
python paper/make_anonymous_code_zip.py     # supplementary_code.zip
```

- `submission_anonymous.zip`: the manuscript for review. `[anonymous]` class
  option set, comment lines stripped, author/affil/email/ack/funding/roles/data
  arguments replaced by a placeholder, compiled from a clean unpack, and its
  PDF text scanned for names. Upload as the main manuscript.
- `supplementary_code.zip`: code, table scripts and every result CSV/JSON with
  a fresh README; every file is scanned for identifying strings and the build
  refuses on a hit. Unpacked alone it regenerates `tables_auto.tex` and
  `macros_auto.tex` byte-identical to the manuscript's. Upload as
  supplementary material for review.
- `manuscript_named.zip`: the same manuscript with authors, for Overleaf and
  for the final files after acceptance.

Locally, `main.tex` builds the named PDF and `main_anonymous.tex` (which only
passes the option and inputs `main.tex`) builds `build/main_anonymous.pdf`.
The body's code link and the supplementary-data notice switch on the same
option through `\codestatement` and `\anonsuppdata`, so there is exactly one
source of text. The public GitHub repository and its named `main.pdf` are
findable by searching the title; making the repository private during review
is the corresponding author's call.

Every archive is one flat folder with IOP-safe file names (IOP's guidelines
forbid subfolders and any character other than letters, digits and
underscore). Rebuild all three after every `make_tables.py` run, since they
carry copies of the generated files.
...
- [x] **Double-anonymous review chosen**, 2026-09-15; anonymous manuscript and
      anonymised code archive built and scanned for names.
- [ ] **Decide whether to make the GitHub repository private during review.**
      It is public and holds the named PDF, so the title is searchable.
